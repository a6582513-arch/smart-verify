...
# ============================================================
#  إعداد Google Gemini API (مجاني) لتشغيل مساعد "AUTH" بذكاء اصطناعي حقيقي
...
#  دون ضبط هذا المتغير، يعمل "AUTH" بالردود الاحتياطية القديمة تلقائياً.
# ============================================================
...
AUTH_SYSTEM_PROMPT = """أنت "AUTH 🛡️"، المساعد الأمني المدمج بمنصة Smart Verify للتحقق الرقمي.
...
def auth_fallback_reply(message: str) -> str:
    """رد احتياطي يعتمد على مطابقة كلمات مفتاحية بسيطة، يُستخدم فقط إذا لم يتوفر
    مفتاح Gemini API أو تعذّر الاتصال به، حتى يبقى المساعد يعمل دون انقطاع كامل."""
    message_lower = message.lower()
    if any(w in message for w in ["من انت", "من أنت", "مين انت", "اسمك", "شو اسمك"]):
        return "أنا AUTH 🛡️، المساعد الأمني المدمج بمنصة Smart Verify. مهمتي مساعدتك بفحص الروابط والرسائل والصور، وتزويدك بخطوات فورية إن مررت بحالة طارئة."
    elif any(w in message for w in ["مرحبا", "اهلا", "أهلا", "السلام عليكم"]):
        return "مرحباً بك، أنا AUTH 🛡️. يمكنني مساعدتك في فحص الروابط والرسائل والصور، أو تقديم إرشادات فورية إن كنت تمر بحالة طارئة."
        ...
        return "أنا AUTH، مساعدك الأمني. اسألني عن الروابط أو الرسائل أو الصور، أو أخبرني إن كنت تمر بحالة أمنية طارئة."
...
def api_chat(data: ChatRequest):
    ...
    if not gemini_client:
        print("[AUTH] GEMINI_API_KEY غير مضبوط أو فشل تحميل مكتبة google-genai")
    elif message:
        try:
            ...
            response = gemini_client.models.generate_content(
                ...
                config=genai_types.GenerateContentConfig(
                    system_instruction=AUTH_SYSTEM_PROMPT,
            ...
            print("[AUTH] Gemini رجع رد فاضي:", response)
        except Exception as e:
            print("[AUTH] خطأ أثناء استدعاء Gemini API:", repr(e))

    return {"reply": auth_fallback_reply(message) if message else "اكتب رسالتك وسأساعدك.", "type": "normal"}
